...
document.addEventListener(
    ...
    function(event) {
        var target = event.target;
        var replyFormcm;
        if (target.matches("[data-toggle='replycm-form']")) {
            replyFormcm = document.getElementById(target.getAttribute("data-target"));
            replyFormcm.classList.toggle("d-none");
        }
    },
    false
);
...
// user details script
    function logfunc() {
      var logf1 = confirm("Do You Want to Signout!!!");
      if (logf1 == true) {
          localStorage.clear();
